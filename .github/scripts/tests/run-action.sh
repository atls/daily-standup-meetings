...
if [[ "$1 $2" == "release verify-asset" ]]; then
  ...
  [[ "$5 $6" == "--repo atls/daily-standup-meetings" ]]
  [[ "${TEST_TAMPERED:-0}" != "1" ]]
...
assert_contains "release download v1.0.0 --repo atls/daily-standup-meetings --pattern dsm-launcher-x86_64-unknown-linux-musl.tar.gz" "${gh_log}"
