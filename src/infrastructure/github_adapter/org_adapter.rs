...
use crate::{
    domain::{org::OrgId, repo::RepoId, repository::OrgRepository},
    graphql_queries::{
        get_org::{get_org::Variables as GetOrgVars, GetOrg},
        get_repo::{
            get_repo::{GetRepoNode, Variables as GetRepoVars},
            GetRepo,
        },
    },
};

use crate::domain::errors::{org::OrgError, repo::RepoError};
...
impl OrgRepository for GitHubAdapter {
    async fn get_org(&self, owner: &str) -> Result<OrgId> {
        ...
        let response = self.client.execute::<GetOrg>(vars).await?;
    ...
    async fn get_repo(&self, org_id: &OrgId, repo_name: &str) -> Result<RepoId> {
        ...
        let response = self.client.execute::<GetRepo>(vars).await?;
